...
Analyze today's git commits to see what Jira tickets harjira would detect.
...
4. Show which tickets would be detected by harjira
5. If no commits today, explain that harjira would find nothing
6. If no Jira tickets in commits, suggest commit message format:
   - `git commit -m "PROJ-123: Description of change"`
   - `git commit -m "[PROJ-123] Description of change"`
7. Optionally show what a dry-run would do: `harjira sync --dry-run`
